...
const sha256Hash = (data) => Buffer.from(sha256(data));
...
const cborEncode = (data) => {
    return Buffer.from(encode$1(data));
};
...
class FountainEncoder {
    ...
    mix(indexes) {
        return indexes.reduce((result, index) => bufferXOR(this._fragments[index], result), Buffer.alloc(this.fragmentLength, 0));
    }
    nextPart() {
