...
describe('Fountain Decoder', () => {
  test('decoder', () => {
    ...
    const encoder = new FountainEncoder(message, maxFragmentLength, 100);
  ...
  test('partition and join', () => {
    ...
    const fragmentsHex = fragments.map(fragment => fragment.toString('hex'));

    const expectedFragmentHexes = [
